...
C = Y * Y'; %simply multiply by the transpose
...
coef = U' * x; % esssentially computes (x - x_bar) * ui
...
%compute coefficient of training set just like we did the normal coef
coef_train = U' * Y; %training set is stored in Y after subtracting mean
...
for i = 1:n_test
    ...
    %first we have to subtract the mean from the text
    updatedTest = img_test(:) - x_bar;
    
    %now we can calculate bu multiplying by U' as before
    coef_test = U' * updatedTest;
